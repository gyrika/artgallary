...
function About(){
  return <main className="about-page"><section className="about-hero page-pad"><div><span className="kicker">Artist · Civil & environmental engineering student · Observer</span><h1>Structure meets <em>intuition.</em></h1></div><div className="portrait"><div className="portrait-shape"></div><span>In the studio, Sri Lanka · 2025</span></div></section>
    <section className="about-intro page-pad"><span>01 — Biography</span><h2>Studying Civil and Environmental Engineering at the University of Ruhuna, Sri Lanka, and drawn toward artistic expression, Piyumini combines structure, geometry and observation with emotion, intuition and symbolic storytelling.</h2><div><p>Piyumini is a Sri Lankan contemporary artist and Civil and Environmental Engineering student at the University of Ruhuna. Her work traces the relationship between inner awareness and the environments we inhabit.</p><p>Her engineering studies shape an instinct for balance, structure, material and environmental systems. Against this measured foundation, pigment is allowed to behave intuitively and unpredictably.</p></div></section>
...
function Contact(){
  const sendEmail=e=>{e.preventDefault();const data=new FormData(e.currentTarget);const body=`Name: ${data.get("name")}\nEmail: ${data.get("email")}\nCountry: ${data.get("country")||"Not provided"}\nArtwork: ${data.get("artwork")}\n\n${data.get("message")}`;window.open(gmailComposeUrl(data.get("subject")||"Website inquiry",body),"_blank","noopener,noreferrer")};
  return <main className="contact-page page-pad"><div className="contact-intro"><span className="kicker">The studio · Sri Lanka</span><h1>Begin a<br/><em>conversation.</em></h1><p>For acquisitions, exhibitions, commissions and collaborations, please contact the studio.</p><div><a href={gmailComposeUrl("Website inquiry","Hello Piyumini,\n\n") } target="_blank" rel="noreferrer">piyuminiabeysundara@gmail.com ↗</a><a href="#">Instagram ↗</a></div></div>
    <form onSubmit={sendEmail}><div className="field-row"><label>Name<input name="name" required placeholder="Your full name"/></label><label>Email<input name="email" required type="email" placeholder="you@email.com"/></label></div><div className="field-row"><label>Country<input name="country" placeholder="Where are you based?"/></label><label>Subject<select name="subject"><option>Artwork acquisition</option><option>Gallery & exhibition</option><option>Commission</option><option>Collaboration</option></select></label></div><label>Artwork of interest<select name="artwork"><option>Please select</option>{artworks.map(a=><option key={a.id}>{a.title}</option>)}</select></label><label>Message<textarea name="message" required rows="6" placeholder="Tell us how we can help…"/></label><button className="button dark">Continue in Gmail <ArrowRight/></button></form>
  </main>
}

function Cart({cart,remove}){
 const inquiryBody=`Hello Piyumini,\n\nI would like to inquire about:\n${cart.map(a=>`• ${a.title}`).join("\n")}\n\nPlease share further details.`;
 return <main className="subpage cart-page page-pad"><div className="page-hero"><span className="kicker">Private acquisition</span><h1>Your selection</h1></div>{!cart.length?<div className="empty"><ShoppingBag/><h2>Your selection is empty</h2><p>Discover original works available directly from the studio.</p><a className="button dark" href="#/artworks">Explore artworks</a></div>:<div className="cart-layout"><div>{cart.map(a=><article key={a.id}><ArtImage art={a}/><div><h2>{a.title}</h2><p>{a.medium}<br/>{a.size}</p></div><button onClick={()=>remove(a.id)}><X/></button></article>)}</div><aside><h3>Selected artworks</h3><p><span>Certificate of authenticity</span><b>Included</b></p><p><span>Professional packaging</span><b>Available</b></p><p><span>Shipping</span><b>Arranged by studio</b></p><a className="button dark full" target="_blank" rel="noreferrer" href={gmailComposeUrl("Artwork acquisition inquiry",inquiryBody)}>Continue inquiry in Gmail</a><small>A prepared email will open in a new tab.</small></aside></div>}</main>
}
